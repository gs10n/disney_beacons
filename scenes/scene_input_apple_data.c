...
static void disney_beacons_app_scene_menu_apple_callback(void* context, uint32_t index) {
...
static const AppleMenuOption apple_menu_options[] = {
    {
        ...
        .data =
            (const uint8_t[]){
                0x1E, 0xFF, 0x4C, 0x00, 0x07, 0x19, 0x05, 0x00, 0x55, 0x10, 0x00,
                0x00, 0x01, 0x4E, 0xCA, 0x91, 0x89, 0xF2, 0xE6, 0x8A, 0xA8, 0xC7,
                0xDC, 0x1E, 0x4A, 0xC7, 0xD3, 0x31, 0xEF, 0x52, 0xD3,
            },
        .data_len = 31,
    },
};
...
void disney_beacons_app_scene_input_apple_data_on_enter(void* context) {
    ...
    for(size_t x = 0; x < apple_menu_options_count; x++) {
        submenu_add_item(
            apple,
            apple_menu_options[x].name,
            x,
            disney_beacons_app_scene_menu_apple_callback,
            ble_beacon);
    }

    view_dispatcher_switch_to_view(ble_beacon->view_dispatcher, DisneyBeaconsAppViewApple);
}
