...
static bool disney_beacons_app_custom_event_callback(void *context,
                                                     uint32_t event) {
  furi_assert(context);
...
static void disney_beacons_app_restore_beacon_state(DisneyBeaconsApp *app) {
  ...
  if (config) {
    ...
    // No config, set up default values - they will stay until overriden or
    // device is reset
    ...
    memcpy(local_config->address, furi_hal_version_get_ble_mac(),
           sizeof(local_config->address));
...
static DisneyBeaconsApp *disney_beacons_app_alloc(void) {
  ...
  app->scene_manager =
      scene_manager_alloc(&disney_beacons_app_scene_handlers, app);
  ...
  view_dispatcher_add_view(app->view_dispatcher, DisneyBeaconsAppViewSubmenu,
                           submenu_get_view(app->submenu));

  app->dialog_ex = dialog_ex_alloc();
  view_dispatcher_add_view(app->view_dispatcher, DisneyBeaconsAppViewDialog,
                           dialog_ex_get_view(app->dialog_ex));

  app->byte_input = byte_input_alloc();
  view_dispatcher_add_view(app->view_dispatcher, DisneyBeaconsAppViewByteInput,
                           byte_input_get_view(app->byte_input));

  app->ears = submenu_alloc();
  view_dispatcher_add_view(app->view_dispatcher, DisneyBeaconsAppViewEars,
                           submenu_get_view(app->ears));

  app->band = submenu_alloc();
  view_dispatcher_add_view(app->view_dispatcher, DisneyBeaconsAppViewBand,
                           submenu_get_view(app->band));

  app->apple = submenu_alloc();
  view_dispatcher_add_view(app->view_dispatcher, DisneyBeaconsAppViewApple,
                           submenu_get_view(app->apple));
...
void disney_beacons_app_update_state(DisneyBeaconsApp *app,
  ...
  furi_check(furi_hal_bt_extra_beacon_set_data(app->beacon_data,
                                               app->beacon_data_len));
